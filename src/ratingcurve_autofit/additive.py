...
def fit_model(
    ...
    stage, discharge = np.asarray(stage, float), np.asarray(discharge, float)
    if stage.ndim != 1 or discharge.ndim != 1 or stage.shape != discharge.shape:
        raise ValueError("Stage and discharge must be one-dimensional arrays of equal length.")
    if len(stage) < 6 or not np.isfinite(stage).all() or not np.isfinite(discharge).all():
        raise ValueError("At least six finite stage/discharge pairs are required for a fit.")
    if np.any(discharge <= 0) or np.unique(stage).size < 4:
        raise ValueError("Additive fitting requires positive discharge and four distinct stage values.")
    if n_segments not in (1, 2, 3):
        raise ValueError("Additive fitting supports one, two or three segments.")
...
def validation_folds(data: pd.DataFrame, folds: int) -> tuple[list[np.ndarray], str]:
    ...
    if complete_dates:
        groups = pd.to_datetime(data[DATE_COLUMN]).dt.normalize().to_numpy()
        dates = np.unique(groups)
        if len(dates) < 2:
            return [], "date-blocked unavailable (one date group)"
        return [np.flatnonzero(np.isin(groups, block))
                for block in np.array_split(dates, min(folds, len(dates)))], "date-blocked"
...
def cross_validate_model(data: pd.DataFrame, fit: dict, folds: int) -> dict:
    ...
    for fold_number, test_idx in enumerate(test_folds):
        ...
        if np.sum(train_mask) < max(6, 3 * fit["n_segments"] + 2):
            failures.append(f"fold {fold_number + 1}: insufficient training observations")
            continue
        try:
            fold_fit = fit_model(
                stage[train_mask], discharge[train_mask], fit["n_segments"],
                seed_offset=5000 + fold_number, global_search=True,
            )
        except (ValueError, RuntimeError, FloatingPointError) as exc:
            failures.append(f"fold {fold_number + 1}: {exc}")
            continue
        if not fold_fit["success"] or not fold_fit.get("empirically_supported", True):
            failures.append(f"fold {fold_number + 1}: unsuccessful or unsupported fit")
            continue
        predicted = predict_discharge(fold_fit["params"], stage[test_idx], fit["n_segments"])
        valid = np.isfinite(predicted) & (predicted > 0.0)
        if not valid.all():
            failures.append(f"fold {fold_number + 1}: nonpositive/nonfinite held-out predictions")
            continue
        predictions[test_idx] = predicted
    count = int(np.isfinite(predictions).sum())
    if count != len(data) or not test_folds:
        return {"method": method, "n": count, "rmse_log10": float("inf"),
                "complete": False, "failures": failures or ["no usable validation groups"]}
    observed, predicted = discharge, predictions
...
def read_input_csv(path: Path) -> tuple[pd.DataFrame, dict]:
    ...
    invalid_stage = int((~np.isfinite(stage_numeric)).sum())
    invalid_discharge = int((~np.isfinite(discharge_numeric)).sum())
    ...
    valid = np.isfinite(stage_numeric) & np.isfinite(discharge_numeric) & (discharge_numeric > 0.0)
...
def save_tables(
        ...
        comparison_rows.append(
            {
                ...
                "cv_complete": fit.get("cv_complete", False),
                "cv_failures": "; ".join(fit.get("cv_failures", [])),
    ...
    (out_dir / "best_model.json").write_text(json.dumps(json_safe(best_json), indent=2, allow_nan=False), encoding="utf-8")
...
def run(
    ...
    for fit in fits:
        ...
        fit["cv_n"] = cv["n"]
        fit["cv_complete"] = cv.get("complete", False)
        fit["cv_failures"] = cv.get("failures", [])
    ...
    metadata = {"method": "additive", "versions": runtime_versions(),
                "input": {"path": str(input_path), "sha256": hashlib.sha256(input_path.read_bytes()).hexdigest()},
                "settings": {"max_segments": max_segments, "bootstrap_samples": requested_bootstrap,
